...
class uav { 
    double speed; 
    double bank; 
    double E; // initial battery
    double p; // power spent per second;
    public:
    double getSpeed();
    double getR(); // minimum turning radius, that is maximum turning rate;
    double getEnergy();
    double getPower();
};

double uav::getSpeed() {
    speed=30;   // constant air speed 30m/s
    return speed;
}
...
double uav::getR() {
    ...
    if (choice==1) {
        bank=30; // maximum bank angle set to 30 degree
    }else {
        cout<<"enter the bank angle between 15 and 30 degree: "; cin>>bank;
    }
    
    double R=speed*speed/(g*tan(bank/180*M_PI));
    return R;
}

double uav::getEnergy() {
    E=200;   // unit: Wh;
    return E;
}

double uav::getPower() {
    p=100;   // constant air speed 30m/s
    return p;
}
...
vector<vector<double>> getWind(int n) {
    ...
    if (choice==1) {
        ...
        // for random wind direction generation   
        srand(time(0));
...
double uavEnergy (vector<vector<double>>& waypoints, vector<vector<double>> &wind, uav u1) {
    //double va=uavData[0], R=uavData[1], inE=uavData[2], p=uavData[3]; // uav speed, R, initial battery, power rate;
    double va=u1.getSpeed();
    double R=u1.getR();
    double inE=u1.getEnergy();
    double p=u1.getPower();
    double res=inE;
...
int main()
{
    ...
    double res=uavEnergy(waypoints, wind, u1);
